...
fetch("https://jsonplaceholder.typicode.com/posts")
  ...
  .then((posts: IPost[]) => {
    // ? finde den kürzesten Titel gib ihn auf der Konsole aus
    const sortedTitlesByLengthAsc = posts.sort(
      (postA: IPost, postB: IPost) => postA.title.length - postB.title.length
    );
  ...
  .then((posts: IPost[]) => {
    //- finde den längsten Body und gib ihn auf der Konsole aus
    const sortedBodiesByLengthDesc = posts.sort(
      (postA: IPost, postB: IPost) => postB.body.length - postA.body.length
    );
