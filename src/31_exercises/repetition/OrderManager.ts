...
class OrderManager {
  ...
  deleteOrder(orderId: string) {
    // ? index finden
    const indexToDelete = this._orders.findIndex(
      (order: IOrder) => order.id === orderId
    );
    if (indexToDelete === -1) {
      throw new Error(
        `Could not find order with id ${orderId}. Deleting not possible`
      );
    }
  ...
  changeCustomerPhoneSeparator(
    ...
    this._orders.forEach((order: IOrder) => {
      ...
      // ? auf ESNext ändern
      order.customer.phone = order.customer.phone.replaceAll(
        separatorBefore,
        separatorAfter
      );
    });
  ...
  doAllOrderHaveProductsShort(): boolean {
    ...
    const allOrdersHaveProduct = this._orders.every(
      (order: IOrder) => order.products.length > 0
    );
